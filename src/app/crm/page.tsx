...
import { KpiRow } from '@/components/stat-tiles';
...
export default async function DashboardPage({
  ...
  const store = one(params.store) ?? null;
  const range: DateRange = { from, to, store };
  ...
  const storeName = store ? options.stores.find((s) => s.code === store)?.name : null;
    ...
    <main className="mx-auto w-full max-w-[92rem] px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-ink-2">
          Lead-to-sale attribution across Meta, WhatsApp, Google Ads and other sources.
        </p>
        {periodFrom && periodTo && (
      ...
      <KpiRow
        ...
        historicalCustomers={kpis.existingPeople}
        reactivatedCustomers={kpis.existingBuyers}
        reactivatedSales={kpis.existingRevenue}
        newCustomers={kpis.newConverted}
        newCustomerBills={kpis.newBills}
      ...
      <section className="card mt-6 rounded-2xl border border-line bg-surface">
        <div className="flex flex-col gap-4 p-6 pb-4">
          ...
          >
            <FilterBar channels={options.channels} />
          </Suspense>
        </div>
